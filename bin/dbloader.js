...
async function getConnection() {
  return await mysql.createConnection({
    ...
    password: "08042001", //Your DB password
  });
}
...
async function makePostsTable(connection) {
  const [result, _] = await connection.query(
    ...
    `
    CREATE TABLE IF NOT EXISTS projectdb.posts (
      id INT NOT NULL AUTO_INCREMENT,
...
async function makeCommentsTable(connection) {
  const [result, _] = await connection.query(
    ...
    `
    CREATE TABLE IF NOT EXISTS projectdb.comments (
      id INT NOT NULL AUTO_INCREMENT,
...
(async function main() {
  ...
  try {
    ...
    //TODO make sure to change yourdbnamehere
    await connection.query("USE projectdb"); // set new DB to the current DB
